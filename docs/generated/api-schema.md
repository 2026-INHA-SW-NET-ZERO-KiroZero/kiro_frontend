...
> 이 파일은 **백엔드 Swagger(OpenAPI) 스냅샷에서 추출**한 명세다. 연동 시 대조 기준(`docs/API-INTEGRATION.md` 2단계).
...
| Method | Path              | 설명          | 요청                                             | 응답                  |
| ------ | ----------------- | ------------- | ------------------------------------------------ | --------------------- |
| POST   | `/api/v1/uploads` | 이미지 업로드 | query `purpose`, body `multipart`(`file` binary) | `ImageUploadResponse` |
...
> enum `Status` = `OPEN` \| `MENU_PROPOSED` \| `COMPLETED`. enum `CookingSkill` = `HIGH` \| `MEDIUM` \| `LOW`. enum `VoteType` = `A`~`E`.

### 인증·프로필

```
SignupRequest        { email*: string<email>, password*: string, nickname*: string, cookingSkill*: CookingSkill, allergyTags: string[] }
...
JoinIngredientRequest    { ingredientId*: long, count*: number, knownGrams: number }
```
...
```
RecommendationRequest  { reason: string }
...
MenuVoteResponse       { slotId: long, recommendationCount: int, voteSummary: object, confirmed: boolean, confirmedCandidateLabel: string, selectedMenu: SelectedMenuSummaryResponse, nextStatus: Status }
...
```
ImageUploadResponse  { fileUrl, objectKey, contentType, size: long }   # POST body: multipart/form-data, field "file" (binary), query "purpose"
