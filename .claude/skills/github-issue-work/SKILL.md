...
6. **구현 완료 후 브랜치 + 커밋 (항상)** — harness 통과를 확인한 뒤, 묻지 말고 새 브랜치(`feat/issue-{번호}-...` 등)를 만들고 Conventional Commits로 커밋한다. 이미 main이면 브랜치를 먼저 판다. 커밋 메시지엔 `Co-Authored-By` 미포함(전역 규칙). **PR 생성은 사용자가 따로 요청할 때만**(요청 시 base `main`, 본문 `closes #번호`).
   - **PR 생성 시 exec-plan 정리 (항상)** — PR을 만들 때 그 PR이 닫는 이슈의 작업 계획 파일 `docs/exec-plans/{YYYY-MM-DD}-issue-{번호}.md`를 **삭제 커밋에 포함**한다(`git rm`). exec-plan은 작업 중 산출물이라 머지되면 main에 남기지 않는다 → PR이 머지되는 순간 해당 파일이 main에서 함께 사라진다. (작업 중에는 유지, PR 직전에만 삭제)

7. **사용자에게 완료 보고** — 작업이 끝났음을 사용자에게 알리고, 브랜치명·변경 파일·요약을 전달한다.

8. **완료 코멘트** — 변경 파일 목록 + 요약을 이슈 코멘트로 남긴다.

9. **이슈 close 안 함** — PR 머지 시 자동 close된다(PR 본문 `closes #번호`).
